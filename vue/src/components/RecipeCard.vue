  ...
  <b-card no-body v-hover>
    <a :href="clickUrl()">
      ...
      <div class="card-img-overlay w-50 d-flex flex-column justify-content-left float-left text-left pt-2"
           v-if="recipe && (recipe.working_time !== 0 || recipe.waiting_time !== 0)">
        <b-badge pill variant="light" class="mt-1 font-weight-normal" v-if="recipe.working_time !== 0"><i
            class="fa fa-clock"></i> {{ recipe.working_time }} {{ $t("min") }}
        </b-badge>
        <b-badge pill variant="secondary" class="mt-1 font-weight-normal" v-if="recipe.waiting_time !== 0"><i
            class="fa fa-pause"></i> {{ recipe.waiting_time }} {{ $t("min") }}
        </b-badge>
      </div>
    </a>
    ...
    <b-card-body class="p-4">
      <h6>
        <a :href="clickUrl()">
          ...
          <template v-else><span class="blurry-text">{{ randomText() }}</span></template>
        </a>
      </h6>
...
export default {
  ...
  methods: {
    ...
    randomText: function () {
      return Math.random().toString(36).substring(2, Math.floor(Math.random() * 20)) + Math.random().toString(36).substring(2, Math.floor(Math.random() * 20))
    }
  },
  directives: {
    hover: {
